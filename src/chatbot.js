    ...
    switch (command.split(' ')[0]) {
      ...
      case 'help':
        this.commands.forEach((c) => {
          response += `<b>${c.name}</b> : ${c.description}<br/>`;
        });
        break;
      case 'blague':
        response = fetch('https://www.blagues-api.fr/api/type/dark/random', {
          ...
          .then((joke) => joke.json())
          .then((joke) => `${joke.joke}<br/>${joke.answer}`);
        break;
      case 'chat':
        response = fetch('https://api.openai.com/v1/chat/completions', {
          ...
          headers: {
            ...
            Authorization: `Bearer ${'sk-NtvwiwFC5Q5v5A1a5'}${'ZuST3BlbkFJC1eq492iRwheGFOSeA99'}`
          },
          body: JSON.stringify({
      ...
      case 'yoda':
        response = fetch(`https://api.funtranslations.com/translate/yoda.json?text=${command.substring(command.indexOf(' '))}`, {
          ...
          .then((result) => `${result.contents.translated}`);
        break;
      case 'dieu':
